...
data <- rbind(data, data.frame(X=NA, date=as.Date('2020-03-21'), time_reported=NA, cases_cumulative=555, fatalities_cumulative=20, tests_cumulative=NA, source_cases=NA, source_fatalities=NA, source_tests=NA))
...
At the time of this report, Georgia is reporting `r max(georgia$cases)` cases and `r max(georgia.deaths$cases)` deaths.
...
A total of 42 cases reported the onset of symptoms on or before 1 March 2020. 
Records of symptom onset exist for 148 out of 159 (93%) cases recorded on or before before March 15.
Further, two cases were hospitalized on or before March. 1
We therefore assume that $42/(0.93)-2 \approx 43$ persons known to the data set were infectious on March 1.
Additionally, of the cases reporting symptom onset after 2 March, we estimate from the incubation period that 17 had already acquired the infection on 1 March.
We therefore assume $17/(1-0.93) \approx 18$ latent infections on 1 March.
Thus, we initialize the epidemic on March 1 with $43+18=61$ cases that would become known.
...
We therefore assume that on March 1 there were approximately $43/0.11 \approx 391$ total infectious cases, of which $391-43=348$ are mildly symptomatic and therefore potentially less infectious.
Additionally, we assume that there are $18/0.11 \approx 164$ latent cases. 
Thus, we assume that there were $391+164=555$ total infections in the population on March 1.
